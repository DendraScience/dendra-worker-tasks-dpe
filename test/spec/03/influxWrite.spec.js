...
describe('influxWrite tasks', function () {
  ...
  it('should run', function () {
    ...
    return machine.clear().start().then(success => {
      ...
      // Check for defaults
      expect(model).to.have.nested.property('sources.influxWrite_in.some_default', 'default')
    })
  })

  it('should drop csi database', function () {
    return model.private.influx.dropDatabase('ucnrs__ucac_angelo').catch(_ => {})
  })
  ...
  it('should process csi data', function () {
    return helper.loadData(dataFileName.csiOut).then(data => {
      ...
      return new Promise((resolve, reject) => {
        model.private.stan.publish('influxWrite.in', msgStr, (err, guid) => err ? reject(err) : resolve(guid))
      })
    })
  })
  ...
  it('should have loaded csi points', function () {
    return model.private.influx.query('select * from source_tenmin', {
      database: 'ucnrs__ucac_angelo'
    }).then(results => {
      expect(results).to.have.lengthOf(1)
      expect(results).to.have.nested.property('0.time')
      expect(results[0].time.getTime()).to.equal(1545663600000)
      expect(results).to.have.nested.property('0.Extra', 1545663600000)
      expect(results).to.have.nested.property('0.Day_of_Year', 358)
    })
  })

  it('should have loaded decoded points', function () {
    return model.private.influx.query('select * from source_goes_tenmin', {
